...
import { formatMonthParam, montrealLocalToUtc, parseMonthParam } from './monthRange';

describe('parseMonthParam', () => {
  it('parses a valid YYYY-MM and returns inclusive range in UTC', () => {
    const range = parseMonthParam('2026-06', new Date('2026-05-07T10:00:00Z'));
  ...
  it('falls back to the current Montreal month for an invalid param', () => {
    const range = parseMonthParam('not-a-month', new Date('2026-05-07T10:00:00Z'));
  ...
  it('falls back to the current month when null', () => {
    const range = parseMonthParam(null, new Date('2026-05-07T10:00:00Z'));
  ...
  it('exposes prev/next month wrapping years', () => {
    const jan = parseMonthParam('2026-01', new Date('2026-05-07T10:00:00Z'));
    expect(jan.prev).toEqual({ year: 2025, monthIndex: 11 });
    expect(jan.next).toEqual({ year: 2026, monthIndex: 1 });

    const dec = parseMonthParam('2026-12', new Date('2026-05-07T10:00:00Z'));
  ...
  it('handles DST: January 1 has -05:00 offset', () => {
    const utc = montrealLocalToUtc(2026, 0, 1, 0, 0, 0, 0);
