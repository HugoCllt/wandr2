...
- **Declarative multi-section Category page system.** Card-variant choices are now made: the surviving cards are `CoverActivityCard` (full-bleed image, favorite/calendar/price overlaid — the canonical feed/map card), `ClassActivityCard`, `MediaRowActivityCard`, `HeroActivityCard`. `CategoryFeedPage` (hero + one `FeedGrid`, Niveau A), `FeedGrid`, `MapSection` and `HeroSection` already render these. What remains: introduce a `sections: [...]` config consumed by one renderer, then retire the `design-showcase` palette. See `CONTEXT.md` "Category page" / "Design showcase" + the 2026-05-27 architecture grilling. — `src/modules/feed/web/CategoryFeedPage.tsx`, `src/app/(with-sidebar)/design-showcase/page.tsx`.
