...
import type { CalendarEntryDTO, FeedResultDTO } from '@wandr/shared';
...
export function useCalendarEntries(from: string, to: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiJson<CalendarEntryDTO[]>(`/api/calendar?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`),
    staleTime: 60_000,
  });
}
...
function useBookmarkCachePatch() {
  ...
  return {
    ...
    restore: (previous: ReturnType<typeof queryClient.getQueriesData<FeedPages>>) => {
...
export function usePendingReviews() {
  ...
  const from = isoDaysAgo(PENDING_REVIEWS_WINDOW_DAYS, now);
  const to = now.toISOString();

  return useQuery({
    queryKey: ['calendar-pending', from, to],
    queryFn: async () => {
      const entries = await apiJson<CalendarEntryDTO[]>(
